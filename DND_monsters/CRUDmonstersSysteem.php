...
class CRUD{
    ...
    private $html = "";
    private $PDFname = "document.pdf";

    private $XLSX_data = [];
    private $XLSXname = "document.xlsx"; 
    ...
    public function is_afbeeldin($postAfbeelding){
    ...
    public function controllMaxMin($nummer){
        if($nummer <0){
            return 0;
        }
        if($nummer > 30){
    ...
    public function controllText($text){
        return empty($text) ? "-": $text;
    }

    //berekent stat modifier
    private function eigenschappenRekenen($X){
    ...
    public function monstersRaeader(){
    ...
    public function Mijn_monstersRaeader($gebruikersID){
        $serverConnectieData = $this->serverConnectieData;
        try
        {
            // Maak een nieuwe connectie naar de database "dnd_npc"
    ...
    public function monsterRaeader($idMonster,$gebruikersID){
        $serverConnectieData = $this->serverConnectieData;
        // $html = $this->html;
        // $PDFname = $this->PDFname;
        // $XLSX_data = $this->XLSX_data;
        ...
        try
        {
            ...
            while($statement->fetch())
            {
                ...
                <table><thead><tr><th>Ability Score</th>
                    ...
                    <td>(".$this->eigenschappenRekenen($postStrength).")</td>
                    <td>(".$this->eigenschappenRekenen($postDexterity).")</td>
                    <td>(".$this->eigenschappenRekenen($postConstitution).")</td>
                    <td>(".$this->eigenschappenRekenen($postIntellihence).")</td>
                    <td>(".$this->eigenschappenRekenen($postWisdom).")</td>
                    <td>(".$this->eigenschappenRekenen($postCharisma).")</td>
                </tr></tbody></table>
                <br>
                <br>
                ...
                <li><strong>$postActions</strong></li>
                    ...
                    $this->PDFname = $postNaam."[$postDanger].pdf";
                    $this->XLSX_data = ["",$postNaam,$postDanger,$postArmor,$postHP,$postStrength,$postDexterity,$postConstitution,$postIntellihence,$postWisdom,$postCharisma,$postSpeed,$postMasteryBonus,$postImmunities,$postVulnerabilities,$postActions,"".$this->eigenschappenRekenen($postStrength),"".$this->eigenschappenRekenen($postDexterity),"".$this->eigenschappenRekenen($postConstitution),"".$this->eigenschappenRekenen($postIntellihence),"".$this->eigenschappenRekenen($postWisdom),"".$this->eigenschappenRekenen($postCharisma)];
                    $this->XLSXname = $postNaam."[$postDanger].xlsx";
                ...
                if($gebruikersID == $postgebruikersID){
                    echo "<div class='bg-light rounded m-2 p-3'>
                        ...
                        <tr>
                        <td>$postStrength(".$this->eigenschappenRekenen($postStrength).")</td><td>$postDexterity(".$this->eigenschappenRekenen($postDexterity).")</td>
                        <td>$postConstitution(".$this->eigenschappenRekenen($postConstitution).")</td><td>$postIntellihence(".$this->eigenschappenRekenen($postIntellihence).")</td>
                        <td>$postWisdom(".$this->eigenschappenRekenen($postWisdom).")</td><td>$postCharisma(".$this->eigenschappenRekenen($postCharisma).")</td>
                        </tr>
                        <tr>
    ...
    public function monsterUpdate($What,$type,$result,$idMonster, $gebruikersID){
        $serverConnectieData = $this->serverConnectieData;
        try
        {
            // Maak een nieuwe connectie naar de database "dnd_npc"
    ...
    public function monsterDelete($idMonster,$gebruikersID){
        $serverConnectieData = $this->serverConnectieData;
        try
        {
            // Maak een nieuwe connectie naar de database "dnd_npc"
    ...
    public function monsterMaken($postNaam,$postArmor,$postHP,$postStrength,$postDexterity,$postConstitution,$postIntellihence,$postWisdom,$postCharisma,$postSpeed,$postDanger,$postMasteryBonus,$postImmunities,$postVulnerabilities,$postActions,$gebruikersID){
        $serverConnectieData = $this->serverConnectieData;
        try
        {
            // Maak een nieuwe connectie naar de database "dnd_npc"
    ...
    <html>
        ...
        $dompdf->stream($this->PDFname, ["Attachment" => true]);
    }

    public function XLSXmake(){
        // $XLSX_data = $this->XLSX_data;
        // $XLSXname = $this->XLSXname;
        ...
        for($i=1; $i<=15; $i++){
            ...
            $sheet->setCellValue('B'.$i, $this->XLSX_data[$i]);
        }
        for($i=16; $i<=21; $i++){
            $sheet->setCellValue('C'.($i-11), $this->XLSX_data[$i]);
        }
